// worker.js — /transfer shows inline buttons [Refresh] [Full/Simple] with robust callback handling
    ...
    if (p==="/webhook/telegram") {
      ...
      if (msg) {
        ...
        switch (c.name) {
          ...
          case "transfer":    await handleTransfer(env, chatId, {h: n(c.args?.[0])||1, view:"full"}); break;
      ...
      if (cq) {
        ...
        if (!chatId) { await tg(env,"answerCallbackQuery",{callback_query_id:cq.id}); return R("ok"); }
        await env.FPL_BOT_KV.put(K.last(chatId), String(Date.now()));
        const cb = parseCb(cq.data||""); // {ns,act,h,v}
        // show a toast so you SEE callbacks are received
        await tg(env,"answerCallbackQuery",{callback_query_id:cq.id,text:(cb.act==="toggle"?"Toggling…":"Refreshing…"),show_alert:false});
        if (cb.ns==="xfer") {
          const h = Math.max(1, Math.min(3, n(cb.h)||1));
          const view = (cb.v==="simple")?"simple":"full";
          await handleTransfer(env, chatId, {h, view: (cb.act==="toggle"?(view==="full"?"simple":"full"):view)}, cq.message);
        }
        return R("ok");
      }
...
async function handleStart(env, chatId, from){
  ...
  const html = [
    `<b>${esc(`Hey ${f}!`)}</b>`,
    "",
    esc("Use commands; /transfer shows inline buttons:"),
    "",
...
/* ---------- /transfer (view=full|simple) with edit-or-send fallback ---------- */
async function handleTransfer(env, chatId, opts={}, editMsg /* optional message to edit */){
  const h = Math.max(1,Math.min(3, n(opts.h)||1));
  ...
  // Inline keyboard + safe edit-or-send
  const ik = ikTransfer(h, view);
  await editOrSend(env, chatId, html, ik, editMsg?.message_id);
}
...
const ikTransfer=(h,view)=>({
  inline_keyboard:[
    [
      {text:"Refresh", callback_data:`xfer:refresh&h=${Math.max(1,Math.min(3,n(h)||1))}&v=${view==="simple"?"simple":"full"}`},
      {text: view==="full"?"Simple":"Full", callback_data:`xfer:toggle&h=${Math.max(1,Math.min(3,n(h)||1))}&v=${view==="simple"?"simple":"full"}`}
    ]
  ]
});

/* ---------- Telegram helpers ---------- */
async function editOrSend(env, chat_id, html, reply_markup, message_id){
  // try edit first (if we have a message id)
  if (message_id) {
    const er = await tg(env,"editMessageText",{
      chat_id, message_id, text:html, parse_mode:"HTML", disable_web_page_preview:true, reply_markup
    });
    if (er?.ok) return;
    // fallback 1: edit only markup (if text considered "not modified" or too long)
    await tg(env,"editMessageReplyMarkup",{chat_id, message_id, reply_markup}).catch(()=>{});
  }
  // fallback 2: send a new message
  await sendHTML(env, chat_id, html, reply_markup);
}

async function sendHTML(env, chat_id, html, reply_markup){
  // ensure we don’t exceed 4096 chars; trim tail if needed
  const MAX=4096;
  const text = html.length>MAX ? (html.slice(0,MAX-20)+"\n…(trimmed)") : html;
  const p={chat_id, text, parse_mode:"HTML", disable_web_page_preview:true};
  if (reply_markup) p.reply_markup = reply_markup;
  try {
    const r = await tg(env,"sendMessage",p);
    if (!r?.ok) await tg(env,"sendMessage",{chat_id,text:strip(text)});
  } catch {
    await tg(env,"sendMessage",{chat_id,text:strip(text)});
  }
}
...
const parseCb = (data)=>{
  // expected: "xfer:refresh&h=1&v=full" or "xfer:toggle&h=1&v=simple"
  const obj = {ns:"",act:""};
  const [first,...rest] = String(data).split("&");
  const [ns,act] = first.split(":");
  obj.ns = ns||""; obj.act = act||"";
  for (const seg of rest){ const [k,v] = seg.split("="); if(k) obj[k]=v }
  return obj;
};

...
export default {
  async fetch(req, env, ctx) {
    ...
    if (req.method==="GET" && (!p||p==="/")) return new Response("OK");
    ...
    if (p==="/webhook/telegram") {
      ...
      let up; try{up=await req.json()}catch{ return new Response("Bad Request",{status:400})}
      ctx.waitUntil(handle(up, env)); return new Response("ok");
    }
...
const send = (e,chat_id,text,opts={})=>fetch(`${api(e.TELEGRAM_BOT_TOKEN)}/sendMessage`,{method:"POST",headers:{'content-type':'application/json'},body:JSON.stringify({chat_id,text,disable_web_page_preview:true,...opts})}).catch(()=>{});
...
async function initWebhook(origin, env){
  ...
  return new Response(j.ok?`Webhook set to ${origin}/webhook/telegram`:`Failed: ${j.description||JSON.stringify(j)}`,{status:j.ok?200:500});
}

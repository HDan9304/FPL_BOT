// worker.js (compact, /transfer with requested layout)
...
async function handleTransfer(env,m){
  ...
  for(const p of baseP){
    ...
    const ln=`${esc(e.web_name)} ${esc(`(${abbr(e.team)}, ${pos(e.element_type)})`)} ${esc(`£${(sp/10).toFixed(1)} sell`)}`;
    if(e.element_type===1) g.GK.push(ln);
    else if(e.element_type===2) g.DEF.push(ln);
    else if(e.element_type===3) g.MID.push(ln);
    else g.FWD.push(ln);
  }
  ...
  const line1=[`${B("Target GW")} ${esc(String(nextId))}`].join("\n");
...
// safe selling price: prefer API selling_price; else buy + floor((now-buy)/2); then fallback to now/buy/0 (all tenths)

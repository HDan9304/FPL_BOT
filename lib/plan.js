// lib/plan.js — build plans A–D and render them
import { fixtureBadgeForTeam, badgeLine as badgeLineFn } from "./fixtures.js";
import { explainMoveHuman, humanReasonSummary } from "./reasons.js";
...
export function renderPlan(title, plan, nextGW, countsNext, elements, fixtures, CFG){
  const lines = [];
  ...
  if (!plan || !plan.moves || !plan.moves.length) {
    ...
    plan.moves.forEach((m, i) => {
      ...
      const human = explainMoveHuman(m, elements, fixtures, nextGW, CFG?.H ?? 2);
...
export const badgeLine = badgeLineFn;
